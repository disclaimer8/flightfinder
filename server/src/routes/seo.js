...
router.get('/sitemap.xml', async (_req, res) => {
  ...
  try {
    ...
    const countries = p1.enumerateCountryUrls();
    for (const u of countries) {
      urls.push({ loc: `${BASE}${lc(u)}`, changefreq: 'weekly', priority: '0.6', lastmod: today });
    }
    if (process.env.NODE_ENV !== 'test') {
      console.log(`[seo] P1 sitemap: +${airportLandings.length} airport-landing, +${airlineNets.length} airline-network, +${airlineAirports.length} airline-airport, +${alliances.length} alliance, +${countries.length} country`);
    }
  } catch (err) {
