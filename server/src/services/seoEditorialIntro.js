...
function airport(meta, data) {
  ...
  const s1 = `${meta.name || meta.city} (${meta.iata}) is served by ${airlineCount} airline${airlineCount === 1 ? '' : 's'} flying to <strong>${destCount}</strong> non-stop destination${destCount === 1 ? '' : 's'} — <strong>${airlineCount}</strong> carrier${airlineCount === 1 ? '' : 's'} and <strong>${destCount}</strong> route${destCount === 1 ? '' : 's'} in total.`;
